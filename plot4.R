...
## and plots four graphs to look at household power consumption over two dates ((2/1/2007 and 2/2/2007):
...
power1 <- subset(householdPower, Date == "2/1/2007") 
power2 <- subset(householdPower, Date == "2/2/2007")
power <- rbind(power1,power2)
power$datetime <- paste(power$Date, power$Time, sep=" ")
power$datetime <- mdy_hms(power$datetime)
...
##graph 3
par(mar=c(2,4,2,4))
with(power, plot(datetime, Sub_metering_3, ylab="Energy sub metering", xlab="", type="l", col="blue"))
